...
import gui.menus.testsMenus.TestsPopupMenuListener;
import gui.renderers.PrincipalAuthorizationCellRenderer;
import gui.tableModels.TestsTableModel;
import gui.windows.TestMessageWindowPopup;

@SuppressWarnings("serial")
public class TestsTable extends JTable
{
	private MessageEditorController messageEditor;
	private IMessageEditor requestViewer;
	private IMessageEditor responseViewer;
	private TestsTableModel testsTableModel;
	
	public TestsTable(MessageEditorController messageEditor, IMessageEditor requestViewer, IMessageEditor responseViewer)
	{
		super();
		
		this.autoCreateColumnsFromModel = true;
		
		this.testsTableModel = new TestsTableModel();
		this.setModel(this.testsTableModel);
		
		this.testsTableModel.addTableModelListener(new TableModelListener()
		{
			...
			public void tableChanged(TableModelEvent e)
			{
				...
				{
					TestsTable.this.customizeTable();
				}
			}
		...
		this.addMouseListener(new TestMessageWindowPopup(this));
		
		JPopupMenu popupMenu = new JPopupMenu();
		popupMenu.addPopupMenuListener(new TestsPopupMenuListener(this));
	...
	{	
		...
		// Test Name Column
		TableColumn testNameColumn = columnModel.getColumn(0);
		testNameColumn.setPreferredWidth(250);
		DefaultTableCellRenderer leftRenderer = new DefaultTableCellRenderer();
		leftRenderer.setHorizontalAlignment(JLabel.LEFT);
		testNameColumn.setCellRenderer(leftRenderer);
	...
	{
		int index = this.tableRowToMessageId(row);
		int modelCol = this.convertColumnIndexToModel(col);
		
		if(TestsTableModel.isPrincipalColumn(modelCol))
		{
			PrincipalMessage principalMessage = BurpExtender.instance.getAuthorize().getTests().get(index).getPrincipalMessage(TestsTableModel.getPrincipalByColIndex(modelCol).getName());
		...
		{
			return BurpExtender.instance.getAuthorize().getTests().get(index).getMessage();
		}
	}
	...
	@Override
	public TestsTableModel getModel()
	{
		return this.testsTableModel;
	}
}
